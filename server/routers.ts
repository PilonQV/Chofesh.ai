...
import { queueVideoGeneration, retrieveVideo, quoteVideoGeneration, VENICE_VIDEO_MODELS, VIDEO_DURATIONS, VIDEO_RESOLUTIONS, VIDEO_ASPECT_RATIOS } from "./_core/veniceVideo";
  ...
  video: router({
    ...
    queue: protectedProcedure
      .input(z.object({
        ...
        model: z.string().default("wan-2.5-preview-image-to-video"),
        duration: z.enum(["5s", "10s"]).default("10s"),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Check credits (video generation costs more than images)
        // 5s video: 10 credits, 10s video: 20 credits
        const creditCost = input.duration === "5s" ? 10 : 20;
        const creditsService = await import("./_core/credits");
        const hasCredits = await creditsService.hasEnoughCredits(ctx.user.id, creditCost);
        if (!hasCredits) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: `Insufficient credits. ${input.duration} videos cost ${creditCost} credits each. Purchase more credits to continue.`,
          });
        }
        ...
        await creditsService.deductCredits(
          ...
          creditCost,
          "video_generation",
          `Video generation: ${input.model} (${input.duration})`
        );
        ...
        try {
          ...
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: `Video generation failed. Your credits have been refunded. Error: ${error.message || "Unknown error"}`,
          });
        }
      }),
    ...
    retrieve: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          const result = await retrieveVideo(input.queueId);
          
          // If completed, save to database
          if (result.status === "completed" && result.video_url) {
            // Note: We'll need to add a generatedVideos table similar to generatedImages
            // For now, we'll use the generatedImages table with a flag
            await createGeneratedImage({
              userId: ctx.user.id,
              imageUrl: result.video_url, // Store video URL in imageUrl field
              prompt: "Video generation", // We don't have the prompt here, would need to store it
              model: "venice-video",
              status: "completed",
              metadata: JSON.stringify({
                type: "video",
          ...
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: error.message || "Failed to retrieve video status",
          });
        }
      }),

    // Get video generation quote
    quote: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          ...
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: error.message || "Failed to get video quote",
          });
        }
      }),
  }),
